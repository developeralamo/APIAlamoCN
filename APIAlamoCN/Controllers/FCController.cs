...
namespace APIAlamoCN.Controllers
{
    ...
    public class FCController : ControllerBase
    {
        ...
        public FCController(Serilog.ILogger logger, FCRepository repository, IConfiguration configuration)
        {
            Logger = logger;
        ...
        public async Task<ActionResult<ComprobanteResponse>> PostFacturacion([FromBody] FCDTO payload)
        {
            ...
            if (!mapping.LoadMappingFile(AppDomain.CurrentDomain.BaseDirectory + @"\Services\FieldMapFiles\FC.json"))
            {
