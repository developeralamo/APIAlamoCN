...
namespace APIAlamoCN.Controllers
{
    ...
    public class COController : ControllerBase
    {
        ...
        public CORepository Repository { get; }
        public IConfiguration Configuration { get; }

        public COController(Serilog.ILogger logger, CORepository repository, IConfiguration configuration)
        {
            Logger = logger;
        ...
        [HttpPost]
        public async Task<ActionResult<ComprobanteResponse>> PostCompras([FromBody] CODTO payload)
        {
            ComprobanteResponse response = null;
            bool dioError = false;

            var ControllerName = "CO";
            ...
            if (!mapping.LoadMappingFile(AppDomain.CurrentDomain.BaseDirectory + @"\Services\FieldMapFiles\CO.json"))
            {
            ...
            string errorMessage = await Repository.ExecuteSqlInsertToTablaSAR(mapping.fieldMap,
                                                                            ...
                                                                            Configuration["CO:JobName"]);

            if (errorMessage != "")
            {
                dioError = true;

                Logger.Error("Error en CORepository.ExecuteSqlInsertToTablaSAR: {Error}", errorMessage);
        ...
        {
            ComprobanteResponse respuesta = await Repository.GetTransaccion(identificador, "SAR_CORMVH");
